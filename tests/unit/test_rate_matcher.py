...
def test_clarification_answer_resolves_to_knee(catalog):
    """The MRI exchange, end to end: ask -> "which one?" -> "knee" -> priced.

    Gates the `clarify-answered-knee` preview pane, which is generated by
    running exactly this and is the only screen showing turn-taking. That
    pane carries an honest "no eval case" stamp, because demo_scripts.yaml
    cases are single-question and none of them covers a second turn -- so
    this test is what stands behind it, and the pane's comment in
    scripts/generate_preview_fixtures.py points here.

    A bare body part is the realistic answer. A CSR who has just been shown
    three MRIs says "knee", not "MRI Knee" -- and "knee" alone scores against
    Knee Arthroscopy's aliases too, which is why the turn-2 pool is
    restricted to the codes offered rather than re-matched against the whole
    sheet.
    """
    turn1 = match_procedure("M1001 wants an MRI", catalog=catalog)
    assert turn1.status == "NEEDS_CLARIFICATION"
    offered = [c.cpt_code for c in turn1.candidates]
    assert set(offered) == {"70551", "72148", "73721"}

    turn2 = resolve_clarification("knee", offered, turn1.clarifying_question, catalog=catalog)
    assert turn2.status == "MATCHED"
    assert turn2.cpt_code == "73721"
    assert turn2.negotiated_rate == Decimal("1150.00")
