...
def test_signature_or_audience_failure_rejected_403_not_trusted():
    ...
    with (
        patch("app.auth.id_token.verify_token", side_effect=ValueError("Token has expired")),
...
def test_token_missing_email_claim_rejected_403():
    request = _FakeRequest(headers={IAP_JWT_HEADER: "fake.jwt.token"})
    with (
        patch("app.auth.id_token.verify_token", return_value={"sub": "12345"}),
        pytest.raises(HTTPException) as exc_info,
    ):
        get_current_csr(request)
    assert exc_info.value.status_code == 403
...
def test_verify_iap_jwt_wraps_any_failure_uniformly():
    ...
    with (
        patch("app.auth.id_token.verify_token", side_effect=ValueError("boom")),
        pytest.raises(IAPVerificationError),
    ):
