# csrsupport-prod environment. Structurally identical to staging/prod (plan
...
terraform {
  ...
  backend "gcs" {
    bucket = "csrsupport-prod-tfstate"
    prefix = "csrsupport"
  }
}
...
variable "enable_vpc_sc" {
  description = "VPC Service Controls / Access Context Manager is an org-level feature -- unavailable on a standalone project with no GCP Organization (see docs/architecture/cicd-setup.md). Leave false for csrsupport-prod on a personal-account project; set true (and supply access_policy_id) once/if this project is part of an org."
  type        = bool
  default     = false
}
...
module "bff_cloud_run" {
  ...
  project_id             = var.project_id
  region                 = var.region
  service_name           = "csrsupport-bff-prod"
  image                  = var.bff_image
  service_account_email  = google_service_account.bff.email
  # Non-zero in prod (plan §3 hardening): absorbs baseline traffic without
  # cold-start Cloud SQL connection bursts when call volume spikes. dev/
  # staging stay at 0 -- cost over availability there.
  min_instances          = 1
  max_instances          = 10
  vpc_connector_id       = module.networking.vpc_connector_id
  env_vars = {
...
module "frontend_cloud_run" {
  ...
  project_id             = var.project_id
  region                 = var.region
  service_name           = "csrsupport-frontend-prod"
