# csrsupport-staging environment. Structurally identical to staging/prod (plan
...
terraform {
  ...
  backend "gcs" {
    bucket = "csrsupport-staging-tfstate"
    prefix = "csrsupport"
  }
}
...
resource "google_service_account" "migrate" {
  project      = var.project_id
  account_id   = "sa-migrate-staging"
  display_name = "CSRSupport DB migration runner (staging)"
}
...
resource "google_service_account" "bff" {
  project      = var.project_id
  account_id   = "sa-bff-run-staging"
  display_name = "CSRSupport BFF Cloud Run (staging)"
}
...
resource "google_service_account" "frontend" {
  project      = var.project_id
  account_id   = "sa-frontend-run-staging"
  display_name = "CSRSupport frontend Cloud Run (staging)"
}
...
module "bff_cloud_run" {
  ...
  project_id             = var.project_id
  region                 = var.region
  service_name           = "csrsupport-bff-staging"
  image                  = var.bff_image
...
module "frontend_cloud_run" {
  ...
  project_id             = var.project_id
  region                 = var.region
  service_name           = "csrsupport-frontend-staging"
