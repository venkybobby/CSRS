...
3. **Eval harness**: `python evals/run_eval.py --env dev` — run all 5 demo-script cases plus adversarial cases against a deployed dev Agent Engine instance; confirm tool-routing sequence, `response_type`, and dollar figures match expected, and zero unexpected `GUARDRAIL_VIOLATION` events. **Status: deterministic mode done — 16/16 cases passing** against real Postgres, covering the 5 demo-script cases plus the clarify-gate, exclusion-vs-no-rate and date-of-service regressions. **Live mode remains unexecuted**: it needs a deployed Agent Engine resource. Its preflight was verified — with `AGENT_ENGINE_RESOURCE_NAME` unset it now fails hard (exit 1) instead of reporting a false PASS, which is what the old skip-and-succeed behaviour did to the post-deploy gate.
...
5. **Adversarial smoke test**: manually attempt one prompt-injection phrase in the UI and confirm the guardrail message appears (not a fabricated number) and a `GUARDRAIL_VIOLATION` log/metric fires. **Status: not executed — still requires a deployed agent.** The guardrail itself is unit-tested (fabrication detection and formatting-divergence cases), but no injection has been attempted through a live model.
