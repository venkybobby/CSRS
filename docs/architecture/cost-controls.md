# Cost controls — what spends money in `csrs-504922`, and how to stop it
...
```bash
curl -s -o /dev/null -w "%{http_code}\n" -H "Authorization: Bearer $(gcloud auth print-access-token)" "https://us-central1-aiplatform.googleapis.com/v1/projects/375096314532/locations/us-central1/reasoningEngines?pageSize=1"
```
...
```bash
curl -s -H "Authorization: Bearer $(gcloud auth print-access-token)" "https://us-central1-aiplatform.googleapis.com/v1/projects/375096314532/locations/us-central1/reasoningEngines?pageSize=100" | python -c "import sys,json;[print(e['name'].split('/')[-1], e.get('createTime','')) for e in json.load(sys.stdin).get('reasoningEngines',[])]"
```
...
```bash
gcloud run services describe csrsupport-bff-dev --region=us-central1 --project=csrs-504922 --format="value(spec.template.spec.containers[0].env)" | tr ',' '\n' | grep AGENT_ENGINE
```
...
curl -s -H "Authorization: Bearer $TOKEN" "https://us-central1-aiplatform.googleapis.com/v1/projects/375096314532/locations/us-central1/reasoningEngines?pageSize=100" | python -c "import sys,json;e=json.load(sys.stdin).get('reasoningEngines',[]);e.sort(key=lambda x:x.get('createTime',''),reverse=True);[print(x['name']) for x in e[2:]]" > /tmp/engines-to-delete.txt
```
...
```bash
while read -r name; do curl -s -X DELETE -H "Authorization: Bearer $TOKEN" "https://us-central1-aiplatform.googleapis.com/v1/${name}?force=true" -o /dev/null -w "deleted ${name##*/}: %{http_code}\n"; done < /tmp/engines-to-delete.txt
```
...
```bash
gcloud sql instances patch my-db-instance --project=csrs-504922 --activation-policy=NEVER
```
