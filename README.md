...
- **Verified in a real browser**: the frontend was built (`npm run build`, clean) and run against
  ...
  1–5). Both need only a vite dev server: no BFF, no agent, no database. Every dollar figure in
  the demo-script fixtures was produced by running the real calculator against `db/seed` and
  matches the `expected_fields` already pinned in `evals/demo_scripts.yaml`. Re-run the script
  after any change to the result components — a stale screenshot is worse than none, because it
  reads as current.
