...
- **Demo deck**: [docs/demo/steering-cut.html](docs/demo/steering-cut.html) is a nine-slide
  walkthrough for the client's steering committee, generated by
  `python scripts/build_demo_deck.py` from those same screenshots — inlined as data URIs, so the
  file is self-contained and opens from anywhere with no server. It is a *recording source*: the
  narration for each slide lives in `SLIDES` in the build script, is hidden behind <kbd>N</kbd>
  during presentation, and <kbd>S</kbd> shows the whole script on one page for a second monitor.
  The deck is generated for the same reason the screenshots and fixtures are — so it cannot
  quietly disagree with what the components render. Re-run `capture_demo_screenshots.py` first,
  then this. The cut deliberately leads with refusals rather than quotes, and its closing slide
  states what has *not* been done; edit the narration in the script, never the generated HTML.
